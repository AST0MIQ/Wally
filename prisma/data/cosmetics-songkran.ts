/**
 * "สงกรานต์" (Songkran) collection data — a light theme built around the Thai
 * water festival: pale water blue, April gold and jasmine white, with RIPPLE
 * as the tap effect.
 *
 * Single source of truth for both `scripts/seed-songkran-theme.ts` and the
 * `20260910120000_songkran_theme` data migration, so the two cannot drift.
 *
 * The artwork lives in Wally's public Blob store; nothing here uploads.
 */
import type { EquipmentSlot, MediaUsage } from "@prisma/client";
import type { AssetConfigV2 } from "../../src/lib/cosmetics/config";

export const BLOB = "https://pvffworkoolfc7yl.public.blob.vercel-storage.com/cosmetics/songkran";
